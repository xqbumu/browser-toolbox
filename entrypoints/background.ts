...
import { downloadDataUrl, downloadBlob, resolveDownloadPath } from '@/utils/download';
...
export default defineBackground(() => {
  ...
  captureService.onProgress((event: ProgressEvent) => {
    lastProgress = event;
    browser.runtime.sendMessage({ type: '__WXT_PROGRESS__', event }).catch(() => {});
  });
  ...
  browser.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
    ...
    handleRequest(msg, captureService)
      ...
      .catch((err) => sendResponse({ ok: false, error: toErrorMessage(err) } satisfies PopupResponse));
    return true; // 异步响应
  });
...
async function handleRequest(
  ...
  switch (msg.type) {
    ...
    case 'BATCH_URLS': {
      ...
      try {
        const result = await service.batchUrls(msg.payload.urls, msg.payload.config, ctx);
        await maybeDownloadZip(result, msg.payload.config);
        for (const item of result.items) {
          await recordHistory(item, msg.payload.config);
        }
    ...
    default: {
      ...
      return { ok: false, error: `未知请求类型: ${unknown.type ?? 'undefined'}` };
    }
  }
}
