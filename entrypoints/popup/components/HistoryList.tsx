...
export function HistoryList({ onPreview, onCountChange }: Props) {
  ...
  async function load(): Promise<void> {
    setLoading(true);
    try {
      const list = await request<ScreenshotListItem[]>({ type: 'HISTORY_LIST', payload: {} });
      setItems(list);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((it) =>
      [it.fileName, it.title, it.url].some((s) => (s ?? '').toLowerCase().includes(q)),
    );
  }, [items, query]);
  ...
  return (
    <div className="history-list">
      <div className="history-toolbar">
        ...
        />
        <button className="danger" disabled={items.length === 0} onClick={() => void handleClear()}>
          清空
        </button>
      </div>
      ...
      {!loading && filtered.length === 0 && <p className="history-empty">暂无截图记录</p>}
...
function HistoryItem({
  ...
  return (
    <div className="history-item">
      <button className="history-thumb" onClick={() => onPreview(item)} title="预览原图">
      ...
      <div className="history-meta">
        ...
        <div className="history-actions">
          <button onClick={() => onPreview(item)}>预览</button>
          <button onClick={() => onRedownload(item.id)}>重新下载</button>
